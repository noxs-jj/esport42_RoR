class Backoffice::EventController < Backoffice::ApplicationController
  ...
  def show
    ...
    if @event.nil?
      redirect_to backoffice_event_index_path,
                  alert: "Event #{params[:id].to_s} doesn't exist"
    else
  ...
  def create
    ...
    event.save
    if event
      redirect_to backoffice_user_show_path(event.id),
  ...
  def update
    ...
    if @event.nil?
      ...
      if @event.update(event_params)
        redirect_to backoffice_event_show_path(@event.id),
